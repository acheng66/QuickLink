  ...
  <div class="workspace-layout">
    <div class="options-box">
      <div class="option-title flex-box">
        <div>
          <span class="section-label">链接分组</span><span>共 {{ editableTabs?.length }} 组</span>
        </div>
          ...
          <div class="item-box flex-box hover-box" :class="{ selectedItem: selectedIndex === index }"
            ...
            <div class="flex-box">
              ...
              <el-tooltip show-after="500" class="box-item" effect="dark" :content="'查看图表'" placement="bottom-end">
                ...
                <el-icon v-if="!(item.quickLinkCount === 0 || item.quickLinkCount === null)" class="edit"
                  :class="{ zero: item.quickLinkCount === 0 }"
              ...
              <span class="item-length">{{ item.quickLinkCount ?? 0 }}</span>
            </div>
          </div>
        </li>
      </ul>
    ...
    <div class="content-box">
      <div class="table-box">
        ...
        <!-- 表格展示区域 -->
        <el-table class="link-table" :data="tableData" height="calc(100vh - 250px)"
          :header-cell-style="{ background: '#f7f9fd', color: '#667085' }">
          <!-- 数据为空时展示的内容 -->
          <template #empty>
    ...
    <!-- 新建分组弹框 -->
    <el-dialog v-model="isAddGroup" title="新建短链接分组" width="480px">
      <el-form :model="form">
    ...
    <!-- 编辑分组弹框 -->
    <el-dialog v-model="isEditGroup" title="编辑短链接分组" width="480px">
      <el-form :model="form">
        <el-form-item label="分组名称：" :label-width="formLabelWidth">
...
const queryPage = async () => {
  ...
  nums.value = editableTabs.value?.[selectedIndex.value]?.quickLinkCount || 0
...
/* QuickLink workspace visual refresh */
.workspace-layout {
  display: flex;
  height: 100%;
  min-width: 0;
